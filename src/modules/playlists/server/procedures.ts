    ...
    .query(async ({ input, ctx }) => {
      ...
      const data = await db
        ...
        .select({
          ...getTableColumns(videos),
          user: users,
          viewedAt: viewerVideoReactions.viewedAt,
          viewCount: db.$count(videoViews, eq(videoViews.videoId, videos.id)),
          likeCount: db.$count(
            videoReactions,
            and(
        ...
        .where(
          and(
            ...
            cursor
              ? or(
                  lt(viewerVideoReactions.viewedAt, cursor.viewedAt),
                  and(
                    eq(viewerVideoReactions.viewedAt, cursor.viewedAt),
                    lt(videos.id, cursor.id)
                  )
                )
              : undefined
        ...
        .orderBy(desc(viewerVideoReactions.viewedAt), desc(videos.id))
        .limit(limit + 1);
